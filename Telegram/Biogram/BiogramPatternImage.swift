...
public enum BiogramPatternImage {
    ...
    public static func make(pattern: String, color: UIColor, opacity: CGFloat, tileSize: CGFloat = 48) -> UIImage? {
        ...
        return tile.resizableImage(withCapInsets: .zero, resizingMode: .tile)
    }
    
    /// UIColor из BiogramProfileColor
    public static func uiColor(from profileColor: BiogramProfileColor) -> UIColor {
        let b = max(0.1, min(1.5, profileColor.brightness))
        return UIColor(
